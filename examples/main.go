...
func (process *MyProcess) Do(page *cm.Page) {
	...
	/**------------------------------波奇的产品爬虫------------------------------**/
		 if reqUrl.String() == "http://shop.boqii.com/" {
			 ...
			 page.AddElem(cm.NewElementItem("http://www.baidu.com"))
	...
	//			sellNum := filter.ReplaceAllString(s.Find(".c999 em").Text(), " ")
	//
	//			tmp = strings.Join([]string{tmp, sellNum, name, price}, " ")
	//
	//			fmt.Print(s.Find(".btns .gdgid").Attr("gid"))
	//
	//			fileMutex.Lock()
	//			file.WriteString(strings.Join([]string{tmp, "\r\n"}, ""))
	//			fileMutex.Unlock()
...
func main() {
	...
	//spider.AddUrl("http://item.jd.com/1569106.html")
	spider.AddUrl("http://shop.boqii.com/product-2362.html")
